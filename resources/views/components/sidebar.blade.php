...
<aside x-cloak :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        @foreach ($menu as $item)
            ...
            <a href="{{ Route::has($item['route']) ? route($item['route']) : '#' }}" @class([
    ...
    <div class="shrink-0 border-t border-border-subtle p-3">
        <button type="button" @click="$dispatch('open-account-modal')"
            class="flex w-full items-center gap-3 rounded-lg bg-primary-dark px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-primary">
