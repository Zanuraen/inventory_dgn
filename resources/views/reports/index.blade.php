<x-layouts.app>
    ...
    <x-header-banner
        title="Laporan Aset"
        description="Ringkasan dan distribusi aset perusahaan"
    />
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-6">
                ...
                <ul class="space-y-2 text-sm flex-1 w-full">
                    ...
                    @foreach ($perKategori as $i => $item)
                        ...
                        <li class="flex items-center justify-between gap-2">
                            <span class="flex items-center gap-2 text-gray-600">
                                <span class="w-2.5 h-2.5 rounded-full shrink-0" style="background-color: {{ $colors[$i % count($colors)] }}"></span>
                                {{ $item->category->name ?? 'Tanpa Kategori' }}
                            </span>
                            <span class="text-gray-400 text-xs shrink-0">{{ $item->total }} ({{ $pct }}%)</span>
                        </li>
                    @endforeach
                </ul>
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 space-y-4">
        ...
        <div class="flex flex-col sm:flex-row sm:justify-end gap-2">
            <a href="{{ route('reports.export.pdf', request()->query()) }}"
               class="inline-flex items-center justify-center gap-2 border border-gray-300 text-gray-600 text-sm font-medium px-4 py-2 rounded hover:bg-gray-50 transition">
                📄 Download PDF
            </a>
            <a href="{{ route('reports.export.excel', request()->query()) }}"
               class="inline-flex items-center justify-center gap-2 bg-[#0B4A63] text-white text-sm font-medium px-4 py-2 rounded hover:bg-[#093c50] transition">
                📊 Download Excel
            </a>
        </div>
            ...
            @forelse ($assets as $i => $asset)
                ...
                <div class="border border-gray-100 rounded-lg p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            <p class="font-medium text-[#0B4A63] truncate">{{ $asset->name }}</p>
                            <p class="text-gray-400 text-xs">{{ $asset->code_asset }}</p>
                        </div>
                        <span class="shrink-0 inline-block px-2.5 py-1 rounded-full text-xs font-medium {{ $badge }}">
                            {{ $asset->condition_status }}
                        </span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-2 mt-3 text-sm">
                        ...
                        <div>
                            <p class="text-gray-400 text-xs">Pemakai</p>
                            <p class="font-medium">{{ $asset->pengguna ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            @empty
                <div class="border border-gray-100 rounded-lg p-10 text-center text-gray-400 text-xs">
                    Belum ada data aset.
                </div>
            @endforelse
        </div>
